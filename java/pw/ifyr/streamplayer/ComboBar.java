...
public class ComboBar extends SeekBar {
    ...
    public ComboBar(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ComboBar);

        mColor = ta.getColor(R.styleable.ComboBar_color, Color.YELLOW);
        mHighlightColor = ta.getColor(R.styleable.ComboBar_highlightColor, Color.RED);
        mTextSize = ta.getDimensionPixelSize(R.styleable.ComboBar_textSize, 5);

        ta.recycle();

        mThumb = new CustomThumbDrawable(context, mHighlightColor);
        setThumb(mThumb);
        setProgressDrawable(
                new CustomDrawable(
